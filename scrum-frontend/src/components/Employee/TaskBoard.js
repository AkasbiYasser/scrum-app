...
const TaskBoard = () => {
  ...
  const updateTaskStatus = async () => {
    try {
      ...
      await axios.patch(`https://scrumflowpfa.tech/api/manager/tasks/${selectedTask.id}/status`, { status: newStatus }, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const addComment = async (taskId, commentText) => {
    try {
      ...
      await axios.post(`https://scrumflowpfa.tech/api/tasks/${taskId}/comments?employeeId=${employeeId}`, newComment, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const deleteComment = async (taskId, commentId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://scrumflowpfa.tech/api/tasks/${taskId}/comments/${commentId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      fetchComments(taskId);
    } catch (error) {
