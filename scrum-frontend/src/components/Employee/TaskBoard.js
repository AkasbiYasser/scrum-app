...
const TaskBoard = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        ...
        const sprintResponse = await axios.get(`http://20.164.56.175/api/manager/tasks/employee/${employeeId}/sprint-details`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  const updateTaskStatus = async () => {
    try {
      ...
      await axios.patch(`http://20.164.56.175/api/manager/tasks/${selectedTask.id}/status`, { status: newStatus }, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const addComment = async (taskId, commentText) => {
    try {
      ...
      await axios.post(`http://20.164.56.175/api/tasks/${taskId}/comments?employeeId=${employeeId}`, newComment, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const deleteComment = async (taskId, commentId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://20.164.56.175/api/tasks/${taskId}/comments/${commentId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      fetchComments(taskId);
    } catch (error) {
