...
const SprintOverview = () => {
  ...
  useEffect(() => {
    const fetchSprints = async () => {
      try {
        ...
        const response = await axios.get('http://20.164.56.175/api/manager/sprints', {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://20.164.56.175/api/manager/sprints', {
        ...newSprint,
  ...
  const handleUpdateSprint = async () => {
    try {
      ...
      await axios.put(`http://20.164.56.175/api/manager/sprints/${currentSprint.id}`, {
        ...currentSprint,
        startDate: formattedStartDate,
        endDate: formattedEndDate
      }, {
  ...
  const handleDeleteSprint = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://20.164.56.175/api/manager/sprints/${currentSprint.id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
