    ...
    const fetchData = async () => {
      ...
      try {
        const [tasksResponse, sprintsResponse, employeesResponse] = await Promise.all([
          axios.get('http://20.164.56.175/api/manager/tasks', { headers: { Authorization: `Bearer ${token}` } }),
          axios.get('http://20.164.56.175/api/manager/sprints', { headers: { Authorization: `Bearer ${token}` } }),
          axios.get('http://20.164.56.175/api/employees', { headers: { Authorization: `Bearer ${token}` } }),
        ]);
