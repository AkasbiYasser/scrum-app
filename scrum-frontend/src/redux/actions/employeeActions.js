...
export const fetchEmployees = () => {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.get('http://20.164.56.175/api/employees', {
        headers: {
...
export const createEmployee = (newEmployee) => {
  return async (dispatch) => {
    try {
      const response = await axios.post('http://20.164.56.175/api/employees', newEmployee, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
...
export const modifyEmployee = (employeeId, updatedEmployee) => {
  return async (dispatch) => {
    try {
      const response = await axios.put(`http://20.164.56.175/api/employees/${employeeId}`, updatedEmployee, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
...
export const removeEmployee = (employeeId) => {
  return async (dispatch) => {
    try {
      await axios.delete(`http://20.164.56.175/api/employees/${employeeId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
