...
export const fetchEmployees = () => {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.get('http://scrum-backend-service:8080/api/employees', {
        headers: {
...
export const createEmployee = (newEmployee) => {
  return async (dispatch) => {
    try {
      const response = await axios.post('http://scrum-backend-service:8080/api/employees', newEmployee, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
...
export const modifyEmployee = (employeeId, updatedEmployee) => {
  return async (dispatch) => {
    try {
      const response = await axios.put(`http://scrum-backend-service:8080/api/employees/${employeeId}`, updatedEmployee, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
...
export const removeEmployee = (employeeId) => {
  return async (dispatch) => {
    try {
      await axios.delete(`http://scrum-backend-service:8080/api/employees/${employeeId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
